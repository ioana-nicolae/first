	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if !ok {
			if err := handlers.SendBadRequest(w, r, "keys are required",
				models.NewServerError(GetEndurGasDealListKeysRequiredErrorCode, "Keys are required")); err != nil {
				gLogger.Errorln(err)
			}
			return
		}
		...
		for _, key := range keysParamQuery {
			...
			if err != nil {
				if err := handlers.SendBadRequest(w, r, "keys must be float64",
					models.NewServerError(GetEndurGasDealListKeysFormatErrorCode, "keys must be float64")); err != nil {
					gLogger.Errorln(err)
				}
			}
